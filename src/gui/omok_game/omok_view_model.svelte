<script lang="ts">
	...
	const mouseMoved = () => {
		...
		if (last_piece_y == piece_y && last_piece_x == piece_x) return;
	...
	onMount(() => {
		...
		const omok_board = (_p: p5) => {
			...
			board_gui = new OmokBoardView(p, 19, 37, 0.5, 2, board_size_px, '/edward-cullen.jpg');
		};
...
<section class="omok-game-instance">
	...
	<div class="game-side-panel">
		<DebugPanel {mouse_coord} {player_turn} {piece_coord} {victory_status} />
		<ChatRoom />
		<wired-card>
			<span>Lobby Code: <strong contenteditable="true" bind:textContent={lobby_code} /></span>
			<br /><span>Creator: <strong contenteditable="true" bind:textContent={creator_name} /></span>
			<wired-button
				on:click={start_game_button_clicked}
				style="float:right"
				bind:this={start_game_button}>Start Game!</wired-button
			>
		</wired-card>
	</div>
</section>
